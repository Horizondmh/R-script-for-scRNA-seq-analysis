...
library(limma)
library(ggplot2)
scRNA_BL_3M = subset(scRNA_all, SampleType == "BL"|SampleType == "3M")
props <- getTransformedProps(scRNA_BL_3M$label, scRNA_BL_3M$orig.ident, transform="logit")
head(props[3])
pair <- rep(c("018","019","021","022","023"),each=2)
group <- rep(c("X3M","BL"),5)
data.frame(group,pair)
design <- model.matrix(~ 0 + group + pair)
design
mycontr <- makeContrasts(groupX3M-groupBL, levels=design)
propeller.ttest(props, design, contrasts = mycontr, robust=TRUE, trend=FALSE, sort=TRUE)


## DEGs_identification & Pathway_enrichment_analysis------------------------------------------------------------------------------------------------------------------
library(Libra)
library(limma)
library(tidyverse)
sc_merge = scRNA_all
sc_merge$cluster <- Idents(sc_merge)
one_month_bl <- subset(sc_merge, SampleType %in% c("BL", "1M"))
one_month_bl_bc <- subset(one_month_bl, cluster %in% c("B_mem", "B_naive", "Plasma"))
one_month_bl_bc$cluster_meta <- "Bc"
table(one_month_bl_bc$cluster)
table(one_month_bl_bc$cluster_meta)
pseudobulk_data <- Libra::to_pseudobulk(one_month_bl_bc, cell_type_col = "cluster_meta", label_col = "SampleType", min_cells = 3, min_features = 3, replicate_col = "orig.ident")
dge <- edgeR::DGEList(counts = pseudobulk_data$Bc, group = colnames(pseudobulk_data$Bc))
...
meta_limma <- 
  tibble(pair = str_extract(colnames(dge), pattern = "\\d+")) |>
  mutate(condition = str_extract(colnames(dge), pattern = "(?<=:)\\w+"))
designMat <- model.matrix(~ 0 + condition + pair,  data = meta_limma)
my_contrasts <- limma::makeContrasts(condition1M-conditionBL, levels = designMat)
dge_voom <- limma::voomWithQualityWeights(dge, designMat, plot = TRUE)
dge_voom <- dge_voom |>
  limma::lmFit(design = designMat, block = NULL) |>
  limma::contrasts.fit(my_contrasts) |>
  eBayes(robust = TRUE)
topgenes <- limma::topTable(dge_voom, n = Inf, adjust.method = "BH") |>
...
scRNA_harmony <- FindClusters(scRNA_harmony, graph.name = "wsnn", algorithm = 3, resolution = 0.8, verbose = FALSE)
